...
import 'dart:developer';
import 'package:flutter/foundation.dart'; // Low-level utility classes
import 'package:flutter/material.dart'; // Material Design widgets
import 'dart:async'; // Asynchronous programming support
...
import 'time.dart';
import 'ble_home.dart'; // Import ble_home.dart
  ...
  Widget build(BuildContext context) {
    ...
    return MaterialApp(
      ...
      routes: {
        '/addCustomer': (context) => const AddCustomerPage(),
        '/time': (context) => TimePage(),
        '/bleHome': (context) => const BleHomePage(), // Add BLE home route
      },
    );
  }
}
      ...
      floatingActionButton: Column(
        ...
        children: [
          ...
          FloatingActionButton(
            onPressed: () async {
              print('削除ボタンが押されました。');
              await fetchAllRecords(); // Call fetchAllRecords
            },
            child: const Icon(Icons.delete, size: 30),
          ),
          const SizedBox(height: 16),
          ...
          const SizedBox(height: 16),
          // Navigate to ble_home.dart button
          FloatingActionButton(
            onPressed: () {
